...
function ui_combobox($id_gui, $opciones, $selected = "", $clase = "", $estilo = "") {
    ...
    return '<select id="' . $id_gui . '" name="' . $id_gui . '" style="' . $estilo . '" class="' . $clase . '">' . $opciones . '</select>';
}
...
function ui_combobox_o_anios($offset = 0) {
    $opciones = '';
    for ($i = 0; $i < 13; $i++) {
        $opciones .= '<option value="' . (date('Y') - ($i + $offset)) . '">' . (date('Y') - ($i + $offset)) . '</option>';
    }
    return $opciones;
}
...
function Rejilla_Resultados($r, $horizontal = false) {
    ...
    for ($i = 0; $i < $nFilas; $i++) {
        ...
        for ($j = 0; $j < $MaxPorFila; $j++) {            
			// Aca se depende mucho de como funciona PHP con el manejo de ORs
			if ($bSaltar || $f = mysqli_fetch_assoc($r)) {
				$bSaltar = false;
				$colspan = '';
				if ( $f['horizontal'] === "1" ) {
					$nElementos++;
					$nFilas = ceil($nElementos / $MaxPorFila);
					// si ya no cabe (estas imagenes ocupan 2 espacios)
					if (($j + 2) > $MaxPorFila) { 
						$bELEMENTOS .= '<td style="text-align:center;vertical-align:top;"></td>';
						$bSaltar = true;
						continue;
					}
					$j++;
				...
				$bELEMENTOS .= '<td '.$colspan.' style="text-align:center;vertical-align:top;">';
                ...
                if (empty($f['variedad_foto'])) {
                    ...
                    } elseif ($f['horizontal'] === "1") {
                        $lazy_img = imagen_URL('cargando_imagen_horizonal', 353, 234, '');
                        $f['variedad_foto'] = imagen_URL($f['variedad_foto'], 353, 234);
                    } else {
                ...
                $bELEMENTOS .= '<div class="categoria-elemento ' . ($f['tiene_oferta'] > 0 ? 'categoria-elemento-oferta' : '') . '" '.( $f['horizontal'] === "1" ? 'style="width: auto !important;"' : "").'>';
