...
class phMagick_resize{
    ...
    function superthumbnail(phmagick $p , $width, $height){
        $cmd = $p->getBinary('convert');
        $cmd .= ' -define jpeg:size='.$width.'x'.$height.' "' . $p->getSource() .'['.$width.'x'.$height.']" -auto-orient -strip -unsharp 0x.6 -quality 75 "'.$p->getDestination().'"';
        $p->execute($cmd);
        $p->setSource($p->getDestination());
        $p->setHistory($p->getDestination());
        return  $p ;
