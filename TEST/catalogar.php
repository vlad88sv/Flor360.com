...
$c = 'SELECT codigo_categoria, titulo FROM flores_categorias WHERE codigo_menu IN (5) ORDER BY titulo';
$r = db_consultar($c);
...
*/
    ...
    $rp = db_consultar($cp);
    $pagina_actual += 1+mysqli_num_rows($rp);
    ...
    while ($fp = mysqli_fetch_assoc($rp))
    {
        ...
        $im->writeImage('catalogo360/pages/'.$pagina_actual.'.jpg');
        $im->clear();
        $im->destroy();
        unset($im);
