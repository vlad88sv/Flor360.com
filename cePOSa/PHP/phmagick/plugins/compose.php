...
/**
    ...
    function overlay(phmagick $p, $overlayImage, $x = 0, $y = 0, $width = 100, $height = 100, $transparency = 100){
        $cmd   = $p->getBinary('composite');
        $cmd .= ' -dissolve ' . $transparency ;
    ...
    function annotate(phmagick $p, $text, $pointsize, $offset, $fill){
        $cmd  = $p->getBinary('convert');
        $cmd .= ' -pointsize "' . $pointsize . '"';
        $cmd .= ' -font "Arial"';
        $cmd .= ' -fill "' . $fill . '"';
        $cmd .= ' -annotate "' . $offset . '" "' . escapeshellcmd($text) . '"';
        $cmd .= ' "' . $p->getSource() .'"' ;
        $cmd .= ' "' . $p->getDestination() .'"' ;
    ...
    function custom(phmagick $p, $command){
        $cmd  = $p->getBinary('convert');
        $cmd .= ' ' . $command;
        $cmd .= ' "' . $p->getSource() .'"' ;
        $cmd .= ' "' . $p->getDestination() .'"' ;
