...
class phpMyEdit
{
	...
	function myquery($qry, $line = 0, $debug = 0) /* {{{ */
	{
		...
		if (! $ret) {
			...
			echo htmlspecialchars(mysqli_error($this->dbh)),'<hr size="1" />',"\n";
		}
		return $ret;
	} /* }}} */
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function execute() /* {{{ */
	{
		...
		*/
		...
		$tmpArray = array_intersect($_FILES,$_POST);
		foreach ($tmpArray as $campo => $valores)
		{
			if ($valores['error'] == 0)
			{
				$NuevoNombre = sha1(microtime());
				move_uploaded_file($valores['tmp_name'],__BASE__.'IMG/i/'.$NuevoNombre);
				$_POST[$campo] = $NuevoNombre;
			}
		}
