...
case 'enviado':
    ...
    <p><strong>¡Esperamos atenderle nuevamente!</strong></p>
    ';
...
case 'datos_basicos':
    ...
    <p><strong>¡Esperamos atenderle nuevamente!</strong></p>
    ';
    $titulo = 'Datos de su compra [#'.$f['codigo_compra'].$f['salt'].']';
break;
case 'confirmacion_eliminado':
    $buffer = '<p>Este correo se le envia por su compra en <strong>'.PROY_NOMBRE_CORTO.'</strong>. Este correo es para informarle que su pedido ha sido eliminado de nuestro sistema.</p>';
    $buffer .= '<p>Razón de eliminación: _</p>';
    $titulo = 'Datos de su compra [#'.$f['codigo_compra'].$f['salt'].']';
break;
case 'error_entrega':
    list($factura,$f) = SSL_COMPRA_FACTURA($_GET['transaccion']);
...
if(isset($_POST['enviar']) || isset($_GET['envio_rapido']))
{
    ...
    $correo = (isset($_GET['envio_rapido']) ? $f['correo_contacto'] : $_POST['correo']);
    registrar($f['codigo_compra'], 'correo.'.@$_GET['plantilla'], $titulo, $contenido);
    correo($correo, $titulo, $contenido);
    echo '<p>Enviado</p>';
    return;
}
...
echo '<p><strong>Destinatario:</strong> <input type="text" name="correo" value="'.$f['correo_contacto'].'" style="width:200px;" /> '.ui_input('enviar','Enviar','submit').'</p>';
